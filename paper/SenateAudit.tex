...
\section{Points of Trust within the Senate Counting Process}
One of the problems with adopting a new Senate counting process is that the legislation around the scrutiny of the count has not kept pace with the technology and processes deployed to perform the count. As a result, the scruineering has lost a significant portion of its value. With the adoption of a new counting process the scrutineering procedures needed to be updated to target different aspects of the system. As a result of this not occurring, the current approach remains in compliance with the legislative requirements, but fails to provide actual assurance of correct operation. 
...
As a result of a lack of scrutiny the current implementation results in a number of points of trust, in which the integrity of the data is not checked and is dependent on the honest and error free operation of the respective software components. We shall highlights those points below: 

\begin{description}
 \item[Image Scanning] There appears to be no verification that the scanned image is an accurate representation of the paper ballot. As such, a malicious, or buggy, component could alter or reuse a scanned image, which would then be utilised for both the automatic and manual data entry. This would pass all subsequent scrutiny, whilst not being an accurate representation of the paper ballot.
 \item[Ballot Data Storage] Whilst a cryptographic signature is produced at the end of the scanning \& processing stage, and prior to submission to the counting system, this signature is based on whatever is in the database. There is no scrutiny that the database accurately represents what was produced by the automatic recognition or the manual operator. An error, or malicious component, with access to the database could undetectably alter the contents. 
...
Modernisation of election counting is, in itself, a good idea. However, as new processes and technology are deployed the auditing and scrutinisation of those processes needs updating. The new points of trust need to be identified and suitable auditing processes developed to allow the overall level of scrutiny to be maintained. This should be performed in an open and transparent manner to instil wider confidence in the system. 
